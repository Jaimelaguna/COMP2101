...
function error-exit {
  if [ $# -gt 0 ] ; then
    error-message
    echo "  Unknown parameter was typed by user"
    errornum=1
  else
    error-message
    echo "  Help was requested by user"
    errornum=0
  fi
  echo ""
  exit $errornum
...
cat <<EOF
  ...
  This script displays all system information, using OPTIONS is able to display a particular specification

  OPTIONS Available for this script:
    -h, --help                 To get this text of help and will exit
    --host                     Show the Host name of the system
    --domain                   Show the domain of the system
    --ipconfig                 Show IP Addresses and interfaces
    --os                       Show the operating system name
    --cpu                      Show cpu specs of the machine
    --memory                   Show the memory of the system
    --disk                     Show the disk name and space of the system
    --printer                  Show the printer name if any is configured

    Any other parameter will exit the program with error
