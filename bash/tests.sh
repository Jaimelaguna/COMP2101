...
test -f /etc/resolv.conf && echo "/etc/resolv.conf is a regular file" || echo "/etc/resolv.conf does not a regular file"
test -d /etc/resolv.conf && echo "/etc/resolv.conf is a directory" || echo "/etc/resolv.conf does not a directory"
test -r /etc/resolv.conf && echo "/etc/resolv.conf is a readable file" || echo "/etc/resolv.conf does not a readable file"
test -w /etc/resolv.conf && echo "/etc/resolv.conf is a writable file" || echo "/etc/resolv.conf does not a writable file"
test -x /etc/resolv.conf && echo "/etc/resolv.conf is a executable file" || echo "/etc/resolv.conf does not a executable file"
echo ""
...
[ -x /tmp ] && echo "/tmp has permission granted" || echo "/tmp can not be accessed"
echo ""
...
firstNumber=4
echo "Please type your first number";read firstNumber
secondNumber=7
echo "Please type your second number";read secondNumber
[ $(($firstNumber%2)) -eq 0 ] && echo The first number is an even number || echo The first number is an odd number
[ $(($secondNumber%2)) -eq 0 ] && echo The Second number is an even number || echo The second number is an odd number
...
[ -v USER ] && echo "The variable SHELL exists" && [ -z USER ] && echo "The variable USER contains: $USER"
echo ""
...
a=1
echo "Please type a first string can be number or not";read a
b=01
echo "Please type a second string can be number or not";read b
